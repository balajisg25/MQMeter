...
const App = () => {
  ...
  // Determine if any transaction exceeds the SLA threshold
  const hasSLAFailures = useMemo(() => {
    return transactions.some(tx => tx.slaPass === "Fail");
  }, [transactions]);

  // Determine if any transaction has an error rate above or equal to 4%
  ...
  // Overall performance: good if all transactions are passing SLA and error rate is below 4%
  const overallPerformance = useMemo(() => {
    if (transactions.length === 0) return null;
    return !hasSLAFailures && !hasHighErrorRate ? "Good" : "Bad";
  }, [transactions, hasSLAFailures, hasHighErrorRate]);

  // Speedometer value: 100 for Good, 0 for Bad (you can adjust as needed)
  const speedometerValue = overallPerformance === "Good" ? 100 : 0;

  // Observation message using previous criteria
  const transactionsWithError = transactions.filter(tx => {
    const total = tx.passCount + tx.failCount;
    if (total === 0) return false;
    const errorRate = (tx.failCount / total) * 100;
    return errorRate > 4;
  });
  const overallSLAStatus = transactions.length === 0 
    ? ""
    : transactions.every(tx => tx.slaPass === "Pass" || tx.slaPass === "N/A")
      ? "90th percentile response times are under the SLA threshold."
      : "Some transactions exceed the SLA threshold.";
  const observation = [
    testDetails ? "Load test completed successfully." : "",
    overallSLAStatus,
    transactionsWithError.length > 0 
  ...
  return (
    <Container maxWidth="lg">
      ...
      {/* Speedometer for overall performance */}
      {overallPerformance && (
        <Box sx={{ my: 2, textAlign: 'center' }}>
          <ReactSpeedometer
            ...
            endColor="green"
            textColor="black"
            currentValueText={`${overallPerformance} Performance`}
          />
        </Box>
      )}
